...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def add_mask_prompt(
        ...
        assert obj_id is not None, "obj_id must be provided to add a mask prompt"
    ...
    def add_tracker_new_mask(
        ...
        """Add or replace an object's mask on one frame, then expose merged outputs."""
        ...
        if obj_rank is None:
            ...
            logger.debug(
                f"[rank={self.rank}] Adding new object with id {obj_id} at frame {frame_idx} via mask."
            )
            self.add_action_history(
                inference_state, "add", frame_idx=frame_idx, obj_ids=[obj_id]
            )
            ...
            logger.debug(
                f"[rank={self.rank}] Refining existing object with id {obj_id} at frame {frame_idx} via mask."
            )
            self.add_action_history(
                inference_state, "refine", frame_idx=frame_idx, obj_ids=[obj_id]
            )
